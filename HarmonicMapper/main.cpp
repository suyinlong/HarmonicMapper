...
void help(char * exe )
{
	...
	printf("%s -iterative_harmonic_map  input_mesh output_mesh_with_uv\n", exe );
}
...
void _harmonic_map( const char * _input, const char * _output )
{
	...
	CHarmonicMapper<CHMMesh> mapper( & mesh );
	mapper._map();
	mesh.write_m( _output );
};

//compute harmonic map, between a topological disk to a disk, using iterative method
void _iterative_harmonic_map( const char * _input, const char * _output )
{
	CHMMesh mesh;
	mesh.read_m( _input );
...
int main( int argc, char * argv[] )
{
	...
	if( strcmp( argv[1] , "-iterative_harmonic_map") == 0 && argc == 4 )
	{
		_iterative_harmonic_map( argv[2], argv[3] );
		return 0;
	}
